...
frame names (odom, base_link, map)

filter gains, thresholds, resolutions, etc.
...
Other nodes and RViz ask TF:

“Where is base_link relative to map right now?”

If TF is missing or wrong:

RViz shows errors like “Frame [map] does not exist”

SLAM can’t update the map correctly

Navigation has no idea where the robot is
...
draw a 3D model of your robot

know its size for collision checking

If URDF is missing:

The robot can still move, publish topics, and do SLAM.
...
If the launch file is missing, you can still run nodes manually –
it’s just less convenient and more error-prone.
...
Nodes do all the work.

YAML files tune how they behave.

TF connects all coordinate frames.

URDF describes what the robot looks like.

Launch files start everything correctly in one command.
